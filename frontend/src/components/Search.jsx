...
function Search() {
    ...
    return (
        <div className={`flex justify-end items-center transition-all rounded-full ${searchOpen ? "max-w-[200px] bg-white h-[30px] mr-[5px]" : "max-w-[41px] h-[41px] hover:bg-white"}`}>
            <SearchInput
                myRef={inputRef}
                placeholder="Search..."
                value={inputValue}
                ...
                disabled={!searchOpen}
            />
            <button disabled={searchOpen} onMouseEnter={() => setButtonHover(true)} onMouseLeave={() => setButtonHover(false)} onClick={() => { setSearchOpen(!searchOpen) }} className={`h-full aspect-square transition-all duration-100 ${searchOpen ? "pr-[8px]" : ""}`}>
                {/* Magnifying glass svg */}
                <svg className={`text-[#1D1B20] transition-all duration-100 transform ${buttonHover ? "scale-[90%]" : ""}`} viewBox="0 0 41 41" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
